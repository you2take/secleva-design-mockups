...
> - オリジナルタグ追加（input + 追加ボタン or Enterキー）→ 選択中エリアにピル追加 + カウンタ更新
> - 選択中タグの × → 削除 + カウンタ更新 ＋ **同じタグを候補先頭に戻す**（誤タップリカバリ）
> - 候補タグのタップ → 選択中へ昇格 + 候補から消滅
> - 候補タグの × → 候補から削除（v1では戻し機能なし）
> - 最大10個制限、重複チェック、15文字制限のトースト警告
> - data 属性駆動：`data-tag-list="selected|suggested"` `data-tag-counter` `data-add-custom-tag` 等
>
...
#### ステップ2：プレビュー＋表示位置調整（**全画像で必須**）
選択された写真をその場で表示し、**切り抜きの「見える範囲」をオーナーが調整できる**。

縦長スマホ写真をヒーロー16:9 や1:1にクロップすると、被写体（顔・料理・看板）が中央外れになりがち。アップ後の救済が必須なので **全画像（メインビジュアル・ロゴ・ギャラリー6枚・メニュー写真3枚）で表示位置調整UIを表示**する。

##### モック側の実装状況（02_block-edit.html）
| ブロック | UI設置箇所 | サンプル写真 |
|---|---|---|
| **Block 1 メインビジュアル** | プレビュー直下に常時表示 | `default/00_hero.webp` |
| **Block 2 ロゴ** | アップ済みプレビューの下に常時表示（円形クロップ時の余白調整用） | `shop-logos/T1_clean.webp` |
| **Block 4 ギャラリー** | サムネ6枚タップ → 大プレビュー切替＋連動UIに反映。各サムネが個別に位置を保存（`data-position-y`） | `default/01_plants.webp` 〜 `06_stones.webp` |
| **Block 5 メニュー写真** | 各メニュー行のアコーディオン展開パネル内、プレビュー直下 | `default/menu/01_oil.webp`〜`04_stones.webp` |
...
- **3プリセット**（タップ1発）：上寄せ(0) / 真ん中(50・デフォルト) / 下寄せ(100)
- **スライダー**（微調整）：0-100の範囲で連続値、`object-position: 50% {value}%` を即時反映
- **ピンチ＆ドラッグ風UI**は v2 で検討（学習コストが高く50代リテラシー層に厳しい）
- アスペクト別の挙動
  - ヒーロー（16:9）：縦方向の位置調整が主、横は中央固定
  - ロゴ（1:1 円形）：通常は中央でOK、ロゴ画像の余白調整用に上下スライダーのみ提供
  - ギャラリー / メニュー（1:1）：縦方向の位置調整が主

##### バックエンド保存
```json
{
  "photo_url": "https://cdn.../hero.webp",
  "object_position_y": 30,  // 0-100、デフォルト50
  "uploaded_at": "2026-05-11T..."
}
```
公開ページ側で `style="object-position: 50% {{ photo.object_position_y }}%;"` として動的注入。WebP原画像は変えずに、表示時の見え方だけ変える設計。
...
- **位置調整UIはサムネごとに独立**：`data-position-y` 属性で各サムネの値を保存、サムネタップで連動復元
- 編集エリア内：大プレビュー / 位置調整UI / 削除・並べ替えボタン
- 全体CTA「自分の写真をアップロード（最大6枚）」で複数選択可
...
#### 表示位置の再調整（**いつでも**）
- 編集パネル内に **常時** 「写真の見える範囲を調整」UI を表示（ステップ2と同じUI）
- アップロード直後だけでなく、後日「やっぱり下寄せにしたい」も即対応
- スライダー操作中はリアルタイムでプレビュー更新（`object-position` を JS で即時書き換え）
- 保存タイミング：スライダー手放し後 300ms debounce で自動保存（明示的「保存」ボタンも併設）
