...
> 💡 **モック側でJS実装済み**：`02_block-edit.html` 内のスクリプトで以下のデモが動作する。
> - オリジナルタグ追加（input + 追加ボタン or Enterキー）
>   - **追加先トグル**：「すぐ選択中に」「候補リストに足す」の2択ボタンを切替
>   - 「すぐ選択中に」：選択中エリアに緑ピル追加＋カウンタ更新
>   - 「候補リストに足す」：候補エリア末尾に白ピル追加（タップで後から選択中へ昇格可能）
> - 選択中タグの × → 削除 + カウンタ更新 ＋ **同じタグを候補先頭に戻す**（誤タップリカバリ）
> - 候補タグのタップ → 選択中へ昇格 + 候補から消滅
> - 候補タグの × → **候補から削除も可能**（候補リスト自体をオーナーが整理できる）
> - 最大10個制限、重複チェック、15文字制限のトースト警告
> - data 属性駆動：`data-tag-list="selected|suggested"` `data-tag-counter` `data-add-custom-tag` `data-add-custom-as="selected|suggested"` 等
>
> **リロード時の挙動（モック）**：
> - 初期表示の候補タグ（HTML にハードコードされている12種）は**ページリロードで常に復活**
> - オーナーが追加したカスタムタグや変更状態は揮発（モックのため localStorage 非使用）
> - 本番では DB の `shop.custom_tag_suggestions[]` に永続化し、× 削除も恒久化する
>
> **視覚的フィードバック（CSS animation）**：
> - 追加時：`tagIn` キーフレーム（0.25s、cubic-bezier(0.34, 1.56, 0.64, 1) で軽くオーバーシュート）
> - 削除時：`tagOut` キーフレーム（0.22s、max-width / padding / margin もアニメで畳まれる）
> - カウンタ更新時：`counterBounce`（0.3s、scale 1 → 1.35 → 1）
...
#### ステップ2：X風 クロップモーダル（**全画像でアップロード時に必須**）
ネイティブピッカーで写真を選んだ瞬間、**フルスクリーンのクロップモーダル**が開く。X（Twitter）と同等の操作で「見える範囲」を確定してから反映。**常設の位置調整UIは置かない**（編集画面が煩雑になり、リテラシー低層に厳しい）。

##### UI仕様
```
┌──────────────────────────────────┐
│ [キャンセル]  写真の見える範囲  [確定]│ ← 黒ヘッダ
├──────────────────────────────────┤
│                                  │
│   ┌────────────────────┐         │
│   │ 画像（ドラッグで移動）│         │
...
- **ライブラリ**：Cropper.js 1.6.2（CDN）
- **アスペクト比**は呼び出し元ボタンの `data-aspect` で指定：
  - メインビジュアル：`1.7778`（16:9）
  - ロゴ：`1`（1:1、表示は円形マスク）
  - ギャラリー：`1`（1:1）
  - メニュー写真：`1.6667`（5:3）
- **操作**：
  - 1本指ドラッグで画像を動かす（枠は固定）
  - ピンチで拡大縮小
  - cropBoxResizable/Movable: false → Xスタイル「画像が動く・枠は固定」
- **確定**：`cropper.getCroppedCanvas()` → DataURL → `target.src` に反映
- **キャンセル**：何も変更せずモーダルを閉じる

##### モック側の実装（02_block-edit.html）

各アップロードボタンに `data-upload-trigger / data-target / data-aspect` を付与し、共通の hidden `<input id="upload-input">` と `#crop-modal` が動く。

| ブロック | ボタン | data-target | aspect |
|---|---|---|---|
| **Block 1 メインビジュアル** | プレビュー全体（「タップして写真を選ぶ」） | `#img-main-visual` | 1.7778 |
| **Block 2 ロゴ** | プレビュー右の「変更」 | `#img-logo` | 1 |
| **Block 4 ギャラリー** | 「この枠の写真を選び直す」／「自分の写真をアップロード（最大6枚）」 | `#img-gallery-current` | 1 |
| **Block 5 メニュー写真** | 各メニュー展開パネル内の「自分の写真をアップロード」 | `#img-menu-1` / `#img-menu-2` / `#img-menu-3` | 1.6667 |

ギャラリーの場合：確定後に大プレビュー（`#img-gallery-current`）と同時に **現在選択中のサムネ画像にも同じDataURLを反映** する。

##### バックエンド保存（実装時）
```json
{
  "photo_url": "https://cdn.../hero.webp",
  "crop": { "x": 120, "y": 80, "width": 1200, "height": 675 },
  "uploaded_at": "2026-05-11T..."
}
```
公開ページ側はクロップ済み画像をそのまま表示（or サーバー側で物理クロップしてCDNに保存）。`object-position` 等の表示時微調整は不要（モーダルで確定済み）。

##### 拡張ポイント
任意の要素を `<button data-upload-trigger data-target="#xxx" data-aspect="N">` にするだけで自動的にクロップフローが発動する。`#upload-input` と `#crop-modal` は1個ずつあれば足りる。
...
│ │ DEFAULT バッジ    │ │
│ └──────────────────┘ │
│ [この枠の写真を選び直す] │ ← クロップモーダル起動
│ [この写真を削除][並べ替え] │
└──────────────────────┘

[ 全体CTA: 自分の写真をアップロード（最大6枚） ]
```
- 6枚サムネは番号バッジ付き（1〜6）。タップで下の編集エリアに切替
- **選択中サムネ**：LINE緑border-2 + チェックマークオーバーレイ + 番号バッジが緑に
- 編集エリアの「この枠の写真を選び直す」 → 選択中サムネに対するクロップモーダル起動
- 確定後、大プレビューと該当サムネ画像の両方が同時に更新される
- 全体CTA「自分の写真をアップロード（最大6枚）」も同じクロップモーダルを起動（モックでは1枚ずつ）
...
#### 表示位置の再調整（後からやり直す場合）
- 「変更」ボタン → 写真ピッカー → クロップモーダル（ステップ2と同じUI）
- **同じ写真のまま表示範囲だけ変えたい**場合は、「変更」内に「同じ写真で範囲を選び直す」のサブ動線（実装時 TODO）
- モック側は「変更」ボタンが直接ピッカーを開く設計
